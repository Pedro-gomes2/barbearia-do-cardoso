...
export default function Index() {
  ...
  const { data: servicos = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase
        .from("servicos")
        .select("id, nome, preco, tipo") // Adicionado o campo 'tipo' aqui
        .eq("ativo", true)
  ...
  // Separação local dos dados do menu baseada na nova coluna do banco de dados
  const corteBarba = servicos.filter(s => s.tipo === 'corte_barba' || !s.tipo);
  const servicosExtras = servicos.filter(s => s.tipo === 'extra');
        ...
        <div className="relative z-10 h-full flex flex-col items-center justify-center text-center px-4">
          <div className="mb-6 animate-fade-in flex flex-col items-center">
            <h1 className="text-7xl md:text-9xl tracking-[0.1em] font-heading text-white leading-tight">
              BARBEARIA <br /> <span className="text-primary">Cardoso</span>
            </h1>
            ...
            <p className="mt-8 text-gray-300 font-light text-xl max-w-xl leading-relaxed italic">
              "Agenda para marcação semanal, seu horário é exclusivo!!"
            </p>
          </div>
      ...
      <section className="container max-w-5xl py-24 space-y-32">
        ...
        {servicos.length > 0 && (
          <div className="space-y-20 py-12 relative">
            ...
            {/* SUB-SEÇÃO: CORTE / BARBA */}
            {corteBarba.length > 0 && (
              <div className="space-y-8 relative">
                <div className="absolute -left-20 -top-10 opacity-5 pointer-events-none select-none hidden lg:block">
                  <Scissors className="h-64 w-64 rotate-45" />
                </div>
                
                <h3 className="text-xl md:text-2xl font-heading tracking-widest uppercase text-primary border-b border-border/60 pb-2">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-x-20 gap-y-4">
                  {corteBarba.map((s) => (
                    <div
                      key={s.id}
                      className="flex items-end justify-between py-2 group hover:text-primary transition-all cursor-default border-b border-border/30"
                    >
                      <div className="flex-1 flex items-end overflow-hidden">
                        <span className="font-heading text-xl tracking-wide whitespace-nowrap group-hover:translate-x-1 transition-transform">
                          {s.nome.toUpperCase()}
                        </span>
            ...
            {/* SUB-SEÇÃO: SERVIÇOS EXTRAS */}
            {servicosExtras.length > 0 && (
              <div className="space-y-8 relative pt-8">
                <div className="absolute -right-20 -top-10 opacity-5 pointer-events-none select-none hidden lg:block">
